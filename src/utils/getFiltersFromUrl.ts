...
export const getFiltersFromUrl = (params: { url: string }) => {
  try {
    ...
    const device = filter.searchParams.get("device") as
      | "desktop"
      | "mobile"
      | "tablet"
      | null
    ...
    const from =
      ...
      dayjs().subtract(11, "days").format("YYYY-MM-DD")
    ...
    if (period === "90d") {
      const from =
        filter.searchParams.get("from") || dayjs().subtract(94, "days")

      return {
        ...commonFilters,
        from: dayjs(from).format("YYYY-MM-DD"),
        to: dayjs(from).add(90, "days").format("YYYY-MM-DD"),
      }
    }
